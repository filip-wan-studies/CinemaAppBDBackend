...
CREATE TABLE IF NOT EXISTS Reservations(id_reservation int unsigned not null auto_increment, id_ticket int unsigned not null, date_issued datetime, date_submission datetime not null, id_client mediumint unsigned, constraint primary key(id_reservation), constraint foreign key(id_client) references Client(id_client), constraint check (date_issued>date_submission));
