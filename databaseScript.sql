...
CREATE TABLE IF NOT EXISTS ScreenSeat(Id int unsigned not null auto_increment, RoomId tinyint unsigned, ScreeningsId int unsigned, constraint primary key(Id), constraint foreign key(RoomId) references Rooms(Id), constraint foreign key(ScreeningsId) references Screenings(Id));
...
(Id, TicketId,  SubmissionDate, ClientId, @vEmail)
