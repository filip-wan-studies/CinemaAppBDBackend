...
CREATE TABLE IF NOT EXISTS Employees(id_employee smallint unsigned not null auto_increment, name varchar(40) not null, surname varchar(40) not null, phone_number decimal(11,0), email varchar(50) not null, secret varchar(50) not null, salary mediumint not null check(salary>0), constraint primary key(id_employee));
...
CREATE TABLE IF NOT EXISTS Reservations(id_reservation int unsigned not null auto_increment, id_ticket int unsigned not null, date_issued datetime, date_submission datetime not null, id_client mediumint unsigned not null, constraint primary key(id_reservation), constraint foreign key(id_client) references Client(id_client), constraint check (date_issued>date_submission));
...
# Reservations
LOAD DATA INFILE 'C:/ProgramData/MySQL/MySQL Server 8.0/Uploads/rezerwacje.csv' INTO TABLE Reservations FIELDS TERMINATED BY ';';